...
const promiseWaterfall = require('./');
...
test('run array of promises sequentially with an initialization parameter',
  function(t) {
    t.plan(1);
...
test('run array consisting of zero promises', function(t) {
  ...
  return promiseWaterfall([]).then(function(result) {
